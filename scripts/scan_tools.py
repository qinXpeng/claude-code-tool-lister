...
DASH = "\u2014"
ICON_MCP = "\U0001f50c"
ICON_PLUGINS = "\U0001f9e9"
ICON_COMMANDS = "\u26a1"
ICON_SKILLS = "\U0001f3af"
ICON_CLIS = "\U0001f5a5\ufe0f"
        ...
        if ":" in line and not raw_line.startswith((" ", "\t")):
            ...
            if val in {">", ">-", ">+", "|", "|-", "|+"}:
                ...
                while i < len(frontmatter_lines):
                    ...
                    if stripped and not block_line.startswith((" ", "\t")) and ":" in stripped:
                        break
                    block_lines.append(stripped)
                    i += 1
                if val.startswith(">"):
                    val = " ".join(part for part in block_lines if part)
                else:
...
def one_line(value: str) -> str:
    """Collapse whitespace for compact markdown output."""
...
def scan_skills(skills_dir: Path, source: str = "") -> list[dict]:
    ...
    for d in sorted(skills_dir.iterdir()):
        ...
        desc = one_line(frontmatter.get("description") or meta.get("description", ""))
...
def scan_all_skills(skill_roots: list[tuple[Path, str]] | None = None) -> list[dict]:
    """Scan Claude and agent skill directories."""
    if skill_roots is None:
        skill_roots = [
            (CLAUDE_HOME / "skills", "~/.claude/skills"),
            (AGENTS_HOME / "skills", "~/.agents/skills"),
        ]

    results = []
    for skills_dir, source in skill_roots:
        results.extend(scan_skills(skills_dir, source=source))
    return sorted(results, key=lambda s: (s["name"].lower(), s["source"]))
...
def scan_commands(commands_dir: Path) -> list[dict]:
    ...
    for f in sorted(commands_dir.glob("*.md")):
        ...
        try:
            ...
            frontmatter = parse_yaml_frontmatter(content)
            desc = one_line(frontmatter.get("description", ""))
        except OSError:
            pass
        results.append({
            "name": f"/{f.stem}",
...
def main():
    ...
    print(f"*Generated by tool-lister skill {DASH} {len(skills)} skills, {len(commands)} commands, {len(plugins)} plugins, {len(mcps)} MCPs, {len(clis)} system CLIs*\n")

    print(f"## {ICON_MCP} MCP Servers\n")
    if mcps:
    ...
    print(f"## {ICON_COMMANDS} Slash Commands\n")
    if commands:
        for c in commands:
            print(f"- **{c['name']}** {DASH} {c['description']}")
    else:
        print("*(none)*")
    print()

    print(f"## {ICON_SKILLS} Skills\n")
    if skills:
        for s in skills:
            v = f" (v{s['version']})" if s['version'] else ""
            source = f" `{s['source']}`" if s["source"] else ""
            print(f"- **{s['name']}**{v}{source} {DASH} {s['description']}")
    else:
        print("*(none)*")
    print()

    print(f"## {ICON_CLIS} System CLIs\n")
    if clis:
